...
async function askGemini(text, userId) {
  try {
    // 獲取用戶的對話歷史
    const conversation = await conversationService.getUserConversation(userId);
    ...
    contents.push({
      role: 'user',
      parts: [{ text: '請使用繁體中文回答。在回答中不要提到你是AI或語言模型。不要加入過多的解釋或前言。' }]
    });
    
    // 添加機器人的回應
    contents.push({
      role: 'model',
      parts: [{ text: '好的，我會使用繁體中文，直接回答問題，不會提及自己是AI。' }]
    });
    ...
    console.log('發送到Gemini的對話內容:', JSON.stringify(contents));
    ...
    // 將AI的回應添加到對話歷史
    await conversationService.addMessageToConversation(userId, 'assistant', resp);
...
async function handleEvent(event) {
  ...
  // 處理「清除記憶」命令
  if (userText === '清除記憶' || userText === '忘記對話') {
    await conversationService.clearUserConversation(userId);
    return { type: 'text', text: '已清除所有對話記憶。' };
  }

  // 在調用askGemini時傳遞userId
  const reply = await askGemini(userText, userId);
